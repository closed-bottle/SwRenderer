...
#ifdef USE_SIMD
inline void MatrixVectorMul(const __m128 &_c0, const __m128 &_c1,
                            const __m128 &_c2, const __m128 &_c3,
                            const float *_x, const float *_y, const float *_z,
                            const float *_w, float *_out) {
...
#endif
template <typename T>
const T *SampleData(const uint8_t *_data, const uint64_t &_offset,
                    const uint64_t &_i) {
  return reinterpret_cast<const T *>(_data + _offset + (sizeof(T) * _i));
}

inline void ClipToNdc(Lamp::Vec4f &_v) { _v /= _v.w; }
...
template <typename T>
T EdgeFunc(const Lamp::Vec4f &_v0, const Lamp::Vec4f &_v1,
...
inline bool BackfaceCullCCW(const double &_cross) { return _cross > 0.0; }

inline bool BackfaceCullCW(const double &_cross) { return _cross <= 0.0; }
  ...
  case LoadOp::LOAD_OP_CLEAR:
    ...
    for (int i = 0; i < _height; ++i) {
      for (int j = 0; j < _width; ++j) {
        void *color_ptr =
            target.Data() + (target.Width() * static_cast<uint32_t>(y + i) +
                             static_cast<uint32_t>(x + j)) *
                                target.Stride();
...
void DrawPointShader(const RenderCmdInfo &_cmd_info) {
  ...
  for (uint64_t i = 0; i < _cmd_info.vertex_buffer_[0]->count_; ++i) {
    auto v3 = *(reinterpret_cast<const Lamp::Vec3f *>(
                    _cmd_info.vertex_buffer_[0]->Data()) +
                (sizeof(Lamp::Vec3f) * i));
    ...
    if (v4.x >= view_port->x && v4.x < view_port->x + uiwidth &&
        v4.y >= view_port->y && v4.y < view_port->y + uiheight) {
...
inline void plotLine(const RenderCmdInfo &_cmd_info, const Lamp::Vec4f &_start,
                     const Lamp::Vec4f &_end) {
...
void DrawTriangleLineShader(const RenderCmdInfo &_cmd_info) {
  ...
  // SIMD version will use more memory footprint because it uses pre-processed
  // vertex. It might need alloc-dealloc every draw, so might be good choice to
  // just allocating it during initialize.

  // alignment and offset is already calculated for vertex.
  auto preprocess_size = sizeof(Lamp::Vec4f) *
                         (vertex_buffer[0]->alloc_count_ + SIMD_REGISTER_WIDTH);
  auto preprocess = Memory(preprocess_size);
  memset(preprocess.Data(), 0, preprocess_size);
  uint64_t offset =
      (SIMD_REGISTER_WIDTH -
       reinterpret_cast<uint64_t>(preprocess.Data()) % SIMD_REGISTER_WIDTH);
  ...
  for (uint64_t i = _cmd_info.first_index_; i < index_buffer[0]->count_; ++i) {
    start =
        std::min(start, *(static_cast<uint32_t *>(index_buffer[0]->data_) + i));
  ...
  __m256 merged[16] = {_mm256_broadcast_ss(&merged_mat.c0.x),
  ...
  for (uint64_t i = start; i < end && j < count; i += 8, j += 8) {
    ...
    memcpy(&raster_data[sizeof(float) *
                        ((1 * vertex_buffer[0]->alloc_count_) + j)],
           &out_y, sizeof(__m256));
    memcpy(&raster_data[sizeof(float) *
                        ((2 * vertex_buffer[0]->alloc_count_) + j)],
           &out_x, sizeof(__m256));
  }
  ...
  for (; j < vertex_buffer[0]->count_; ++j) {
    ...
    memcpy(
        &raster_data[sizeof(float) * (1 * vertex_buffer[0]->alloc_count_ + j)],
        &v0.y, sizeof(float));
    memcpy(
        &raster_data[sizeof(float) * (2 * vertex_buffer[0]->alloc_count_ + j)],
        &v0.z, sizeof(float));
    memcpy(
        &raster_data[sizeof(float) * (3 * vertex_buffer[0]->alloc_count_ + j)],
        &v0.w, sizeof(float));
  }
  ...
  uint8_t *y =
      &raster_data[sizeof(float) * (1 * vertex_buffer[0]->alloc_count_)];
  uint8_t *z =
      &raster_data[sizeof(float) * (2 * vertex_buffer[0]->alloc_count_)];
  uint8_t *w =
      &raster_data[sizeof(float) * (3 * vertex_buffer[0]->alloc_count_)];
...
#else
  auto *vertices = reinterpret_cast<const Lamp::Vec3f *>(
      _cmd_info.vertex_buffer_[0]->Data());
  ...
  for (uint64_t i = start; i <= end; ++i) {
    alignas(16) Lamp::Vec4f v0 =
        Lamp::Vec4f(vertices[i].x, vertices[i].y, vertices[i].z, 1.0f);
    v0 = uniform->mvp * v0;
    ClipToNdc(v0);
    NdcToWindow(viewport_transform, v0);

    memcpy(preprocess.Data() + (i * sizeof(Lamp::Vec4f)), &v0,
           sizeof(Lamp::Vec4f));
  }
...
inline void FillInTriangle(const Lamp::Vec4f &_p, const Lamp::Vec4f &_v0,
                           const Lamp::Vec4f &_v1, const Lamp::Vec4f &_v2,
                           const double &_area, const float &_near,
                           const float &_far, const float &_depth,
  ...
  if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
    ...
    const uint8_t color[] = {
        static_cast<uint8_t>(255.0 * p_interp[0] / z_interp),
    ...
    if (_depth < double_fp_depth) {
      void *color_ptr = _color_target.Data() +
                        (_color_target.Width() * static_cast<uint32_t>(_p.y) +
      ...
      void *depth_ptr = _depth_target.Data() +
                        (_depth_target.Width() * static_cast<uint32_t>(_p.y) +
                         static_cast<uint32_t>(_p.x)) *
...
inline void DrawRasterShader(const RenderCmdInfo &_cmd_info) {
  ...
  const uint64_t offset =
      SIMD_REGISTER_WIDTH -
      reinterpret_cast<uint64_t>(raster_buffer) % SIMD_REGISTER_WIDTH;
  ...
  for (uint64_t i = start; i <= end && out_itr <= count; i += 8, out_itr += 8) {
    ...
    memcpy(&raster_data[sizeof(float) *
                        ((1 * vertex_buffer[0]->alloc_count_) + out_itr)],
           &out_y, sizeof(__m256));
    memcpy(&raster_data[sizeof(float) *
                        ((2 * vertex_buffer[0]->alloc_count_) + out_itr)],
           &out_z, sizeof(__m256));
  }
  auto i_d = static_cast<uint8_t *>(index_buffer[0]->data_);

  auto *backface_culling = _cmd_info.pipeline_->front_face_ == WindingOrder::CCW
                               ? BackfaceCullCCW
                               : BackfaceCullCW;
  ...
  for (uint64_t i = 0; i < index_buffer[0]->count_; i += 3) {
    ...
    AABB2i aabb;
    aabb.min = {static_cast<int>(std::min(std::min(v0.x, v1.x), v2.x)),
                static_cast<int>(std::min(std::min(v0.y, v1.y), v2.y))};
    aabb.max = {static_cast<int>(std::max(std::max(v0.x, v1.x), v2.x)) + 1,
    ...
    aabb.max.x = std::min(aabb.max.x, static_cast<int>(left + width) - 1);
    aabb.max.y = std::min(aabb.max.y, static_cast<int>(top + height) - 1);

    LAMPASSERT(aabb.max.x < 0, "AABB Out of bound");
    LAMPASSERT(aabb.max.y < 0, "AABB Out of bound");

    // Near/far plane clipping
    if (aabb_depth < view_port->near || aabb_depth > view_port->far) {
      continue;
    }

    // Cross product == Area of parallelogram made with the area of triangle
    // * 2. Note that this edge function basically does pseudo-cross product.
    double area;
    ...
    for (int j = aabb.min.y; j < aabb.max.y; ++j) {
      for (int k = aabb.min.x; k < aabb.max.x; ++k) {
        ...
        void *depth_ptr = depth_target.Data() +
                          (depth_target.Width() * static_cast<uint32_t>(p.y) +
        ...
        FillInTriangle(p, v0, v1, v2, area, view_port->near, view_port->far,
                       stored_depth, color_target, depth_target);
      }
    }
  }
#else
...
inline void FillTexture(const Lamp::Vec4f &_p, const Lamp::Vec4f &_v0,
                        const Lamp::Vec4f &_v1, const Lamp::Vec4f &_v2,
                        const Lamp::Vec2f &_u0, const Lamp::Vec2f &_u1,
                        const Lamp::Vec2f &_u2, const float &_w0,
                        const float &_w1, const float &_w2, const double &_area,
                        const float &_near, const float &_far,
                        const float &_depth, const uint8_t *_heap,
                        const Image &_texture, const Image &_color_target,
                        const Image &_depth_target) {
  // There are some reference about barycentric coordinate in page 479.
  // https://registry.khronos.org/OpenGL/specs/gl/glspec46.core.pdf
  const double w0 = EdgeFunc<double>(_v1, _v2, _p) / _area;
...
inline void DrawTexturedShader(const RenderCmdInfo &_cmd_info) {
  ...
  auto *positions = reinterpret_cast<const Lamp::Vec3f *>(
      _cmd_info.vertex_buffer_[0]->Data());
  auto *uvs = reinterpret_cast<const Lamp::Vec2f *>(
      _cmd_info.vertex_buffer_[1]->Data());
  ...
  for (uint64_t i = _cmd_info.first_index_; i < index_buffer->count_; ++i) {
    start =
        std::min(start, *(static_cast<uint32_t *>(index_buffer->data_) + i));
    end = std::max(end, *(static_cast<uint32_t *>(index_buffer->data_) + i));
  }

  for (uint64_t i = start; i <= end; ++i) {
    alignas(16) auto v0 =
        Lamp::Vec4f(positions[i].x, positions[i].y, positions[i].z, 1.0f);
    auto uv0 = Lamp::Vec2f(uvs[i].x, uvs[i].y);

    v0 = uniform->mvp * v0;
    float one_over_clip = 1.0f / v0.w;
    clip_w.push_back(one_over_clip);
    uv0 *= one_over_clip;
    ClipToNdc(v0);
    NdcToWindow(viewport_transform, v0);
  ...
  for (uint64_t i = 0; i < index_buffer->count_; i += 3) {
    ...
    for (int j = aabb.min.y; j < aabb.max.y; ++j) {
      for (int k = aabb.min.x; k < aabb.max.x; ++k) {
        ...
        FillTexture(p, v0, v1, v2, u0, u1, u2, clip_w[i0], clip_w[i1],
                    clip_w[i2], area, view_port->near, view_port->far,
                    stored_depth, heap, *_cmd_info.image_[0], color_target,
                    depth_target);
      }
    }
  }
