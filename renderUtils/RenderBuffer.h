...
struct VertexBuffer {
  ...
  uint64_t alloc_count_ =
      0; // Size of count that are actually used to allocate for padding.
#ifdef USE_SIMD
  ...
  VertexBuffer(const size_t _stride, void *_data, const uint64_t _vertex_count)
      : stride_(_stride),
        ...
        mem_(0), count_(0)
#else
        data_(static_cast<uint8_t *>(_data)), count_(_vertex_count)
#endif
  {
    ...
    if (_stride == 12) {
      ...
      for (uint64_t i = 0; i < _vertex_count; i += SIMD_VECTOR_FETCH_PADDING) {
        // 1. Load 8 * Vec3, (8 * 12 bytes), think it as 6 * Vec4(which fits
        // into 128 reg.)
        ...
        auto d256 =
            (__m128 *)(reinterpret_cast<uint64_t>(_data) + (i * _stride));
        ...
        auto merged01 =
            _mm256_shuffle_ps(chunk0, chunk1, _MM_SHUFFLE(1, 0, 2, 1));
        auto merged23 =
            _mm256_shuffle_ps(chunk1, chunk2, _MM_SHUFFLE(2, 1, 3, 2));
        ...
        memcpy(Data() + sizeof(float) * (1 * alloc_count_ + i), &y,
               8 * sizeof(float));
        memcpy(Data() + sizeof(float) * (2 * alloc_count_ + i), &z,
               8 * sizeof(float));
      }
      ...
      for (uint64_t i = last_chunk; i < count_; ++i) {
        ...
        memcpy(Data() + sizeof(float) * (alloc_count_ + i), &v->y,
               sizeof(float));
        memcpy(Data() + sizeof(float) * (2 * alloc_count_) + i, &v->z,
               sizeof(float));
      }
    }
#else
