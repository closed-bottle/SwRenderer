...
inline bool LoadOBJ(const Lamp::String &_path, Geometry &_out_geom,
  ...
  while (getline(input_file, buffer)) {
    ...
    while (buffer_stream >> stream_segment) {
      ...
      } else if (stream_segment == "v") // Geometric vertex
      {
        ...
        // Push back dummies for attributes, so number of element for each
        // vertex attributes matches the number of vertices even if there are
        // duplicates.
  ...
  _out_geom.vertex_normal_ =
      Memory(sizeof(Lamp::Vec3f) * vnormals.size() + valign);
  ...
  _out_mesh.vnormal_offset_ =
      valign -
      (reinterpret_cast<uint64_t>(_out_geom.vertex_normal_.Data()) % valign);
  ...
  memcpy(_out_geom.vertex_.Data() + _out_mesh.vertex_offset_, vertices.data(),
         sizeof(Lamp::Vec3f) * vertices.size());

  if (!vnormals.empty()) {
    memcpy(_out_geom.vertex_normal_.Data() + _out_mesh.vnormal_offset_,
           vnormals.data(), sizeof(Lamp::Vec3f) * vnormals.size());
  }

  if (!uvs.empty()) {
    memcpy(_out_geom.uv_.Data() + _out_mesh.vt_offset_, uvs.data(),
           sizeof(Lamp::Vec2f) * uvs.size());
  }

  if (!indices.empty()) {
    memcpy(_out_geom.index_.Data() + _out_mesh.index_offset_, indices.data(),
